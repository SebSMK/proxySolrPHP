...
require_once(dirname(__FILE__) . '/Log2Solr/Proxy2Solr.php');
...
function solr_proxy_main() {
	...
	if (isset($_GET['solrUrl'])){
		...
		if (isset($_GET['query'])) {
		// save request data in Solr
			$g = new Proxy2Solr('http://csdev-seb:8180/solr-example/dev_search_log/', $_GET['query']);
			$g->run();
		//  $g->flush();
		}
